...
\begin{itemize}
    \item \textbf{Fase 1 (Feature Extraction):} \texttt{backbone.trainable=False}, solo se entrena la cabeza ($\sim$165\,k params), \texttt{lr=1e-3}, 12 épocas.
    \item \textbf{Fase 2 (Fine-Tuning):} se descongelan las últimas 20 capas del backbone, \texttt{lr=1e-5} (1\,000$\times$ menor para no destruir los pesos pre-entrenados de ImageNet), 12 épocas adicionales.
\end{itemize}
...
\begin{table}[H]
\centering
\caption{CNN desde cero (Semana 2) vs Transfer Learning (Semana 3)}
\label{tab:metricas}
\footnotesize
...
\paragraph{Resultado adverso y su análisis.} La CNN regularizada de Semana 2 (entrenada desde cero, solo $\sim$1,2\,M parámetros) \textbf{supera al Transfer Learning con MobileNetV2} ($\sim$3,5\,M parámetros) por +12,2 puntos de F1-macro. Este resultado contradice la expectativa habitual y exige explicación cuidadosa.
...
\subsection{Análisis de ventajas y desventajas observadas}

\textbf{¿Por qué el Transfer Learning fue peor en este caso?} Cuatro causas concurrentes:
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item \textbf{Fine-tuning conservador} (solo 20 capas descongeladas, LR=1e-5). Con un mismatch tan grande, posiblemente convendría descongelar más capas o entrenar más épocas.
\end{enumerate}
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item \textbf{La estrategia metodológica (2 fases con LR diferenciado) funciona como teoría predice.} Las curvas muestran que el fine-tuning con LR muy bajo no destruyó los pesos pre-entrenados — solo refinó marginalmente. El problema no es la metodología sino la elección de backbone.
\end{enumerate}
